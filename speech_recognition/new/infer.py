...
from speech_recognition.new.decoders.decoder import Decoder
...
class InferenceProcessor:
    ...
    def __init__(self, cfg: InferConfig) -> None:
        ...
        # load model
        ckpt = torch.load(self.cfg.common_eval.path, map_location="cpu")
        from wav2vec2.model import wav2vec2_model
        models = wav2vec2_model(**ckpt['config'])
        with torch.no_grad():
