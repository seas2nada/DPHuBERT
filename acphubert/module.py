...
class L0Dense(Module):
    ...
    def l0_norm(self):
        if self.use_bias:
            return (self.qz_loga).sigmoid().sum() * self.out_features + self.out_features
        else:
            return self.qz_loga.sigmoid().sum() * self.out_features

    def forward(self, input):
        if input.dim() == 3:                       # (B, T, D)
            B = input.size(0)
            if self.local_rep or not self.training:
                z = self.sample_z(B, sample=self.training).unsqueeze(1)  # (B,1,D)
                output = torch.matmul(input * z, self.weights)          # (B,T,out)
            else:
                output = torch.matmul(input, self.sample_weights())     # (B,T,out)
        elif input.dim() == 2:                     # (B, D)
            if self.local_rep or not self.training:
...
class L0Conv1d(Module):
    """Hard‑Concrete gated 1‑D convolution analogous to `L0Conv2d`.

    Gating is applied per *output channel*; computations follow Louizos et al.
    Parameters mimic `nn.Conv1d` plus L₀ hyper‑parameters.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.prior_prec = weight_decay
        self.lamba = lamba
        self.droprate_init = droprate_init or 0.5
        self.temperature = temperature
        self.local_rep = local_rep

        self.weights = Parameter(torch.Tensor(out_channels, in_channels // groups, kernel_size))
        self.qz_loga = Parameter(torch.Tensor(out_channels))  # gate per output channel
        self.use_bias = bias
        if bias:
            self.bias = Parameter(torch.Tensor(out_channels))
        self.floatTensor = torch.FloatTensor if not torch.cuda.is_available() else torch.cuda.FloatTensor
        self.input_shape = None  # captured on first forward
        self.reset_parameters()
        print(self)
    ...
    def _reg_w(self):
        """Expected L0 norm under the stochastic gates, takes into account and re-weights also a potential L2 penalty"""
        q0 = self.cdf_qz(0)
        logpw_col = torch.sum(- (.5 * self.prior_prec * self.weights.pow(2)) - self.lamba, 2).sum(1)
        logpw = torch.sum((1 - q0) * logpw_col)
        logpb = 0 if not self.use_bias else - torch.sum((1 - q0) * (.5 * self.prior_prec * self.bias.pow(2) -
                                                                    self.lamba))
        return logpw + logpb
    ...
    def count_expected_flops_and_l0(self):
        """Expected (FLOPs, L0) for this 1‑D conv under the Hard‑Concrete prior."""
        # probability a channel *survives*
        ppos = torch.sum(1 - self.cdf_qz(0))  # scalar

        # mul/adds per output position for one output channel
        n = self.kernel_size * (self.in_channels // groups)  # number of weights per filter
        flops_per_pos = n + (n - 1)            # n mult + (n-1) adds

        # number of positions (time steps) per filter – requires recorded input length
        if self.input_shape is None:
            return torch.tensor(0.), torch.tensor(0.)
        T_in = self.input_shape[2]
        T_out = (
            (T_in + 2 * self.padding - self.dilation * (self.kernel_size - 1) - 1) // self.stride
        ) + 1
        flops_per_filter = flops_per_pos * T_out

        expected_flops = flops_per_filter * ppos
        expected_l0 = n * ppos

        if self.use_bias:
            expected_flops += T_out * ppos  # bias add per output position
            expected_l0 += ppos             # bias weight
    ...
    def sample_z(self, batch_size, sample=True):
        if sample:
            ...
            z = self.quantile_concrete(eps).unsqueeze(-1)
            return torch.clamp(z, 0, 1)
        else:
            pi = torch.sigmoid(self.qz_loga).view(1, self.out_channels, 1)
            return torch.clamp(pi * (limit_b - limit_a) + limit_a, 0, 1)

    def sample_weights(self):
        """Return gated weights with shape (C_out, C_in//groups, k)."""
        z = self.quantile_concrete(self.get_eps(self.floatTensor(self.out_channels))).view(
    ...
    def l0_norm(self):
        if self.use_bias:
            return (self.qz_loga).sigmoid().sum() * self.kernel_size * (self.in_channels // self.groups) + self.out_channels
        else:
            return self.qz_loga.sigmoid().sum() * self.kernel_size * (self.in_channels // self.groups)

    # ------------------------------ forward ---------------------------------
    def forward(self, input_: torch.Tensor):  # (B, C_in, T)
        if self.input_shape is None:
        ...
        if self.local_rep or not self.training:
            ...
            return F.conv1d(input_, w, None, self.stride, self.padding, self.dilation, self.groups)
