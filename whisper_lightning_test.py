...
class DistillModule(pl.LightningModule):
    ...
    def _validation_step(self, batch, batch_idx, mode="valid"):
        ...
        gen_kwargs = {'max_length': 225, 'synced_gpus': False, 'language': self.language}
    ...
    def compute_metrics(self, pred):
        ...
        log_file_prefix = "cv16_" + str(self.language)
        log_file_name = log_file_prefix + "-" + log_file_name

        log_file_path = os.path.join("whisper_test_decodings", log_file_name)
        ...
        wer_file_name = log_file_prefix + "-" + "wer.txt"
        wer_file_path = os.path.join("whisper_test_decodings", wer_file_name)
        with open(wer_file_path, 'w', encoding='utf-8') as f:            
            f.write(str(wer))
    ...
    def on_after_backward(self):
        ...
        if self.param_reg_type == "l2":
            ...
            # Sample a subset of gradients for quantile calculation
            num_samples = int(sample_percentage * len(all_gradients))
            sampled_gradients = torch.randperm(len(all_gradients))[:num_samples]
            sampled_gradients = all_gradients[sampled_gradients]
            sorted_gradients = torch.sort(sampled_gradients.abs())[0]

            # Calculate the threshold for the lowest 20% of gradients using quantiles
            # threshold = torch.quantile(sampled_gradients.abs(), 0.2).item()
    ...
    def validation_step(self, batch, batch_idx):
        loss, wer = self._validation_step(batch, batch_idx, mode="valid")
