...
from whisper_lightning_test import (
    DistillModule,
    DistillLoss,
)
...
class WhisperData:
    def __init__(self, whisper_model_name, language="en", batch_size=8):
    ...
    def prepare_dataloader(self, dataset):
        ...
        vectorized_datasets = dataset.map(
            prepare_dataset,
            num_proc=2,
            cache_file_name="/home/ubuntu/Workspace/huggingface/whisper_test.arrow",
    ...
    def val_dataloader(self):
        cv_16 = load_dataset("mozilla-foundation/common_voice_16_1", self.language.split("+")[0], split="validation")
        return self.prepare_dataloader(cv_16)
...
def run_train(args):
    ...
    student_embed_dim = 1280
    teacher_embed_dim = 1280
    ...
    whisper_data = WhisperData(args.whisper_model_name, language=args.language, batch_size=args.batch_size)
